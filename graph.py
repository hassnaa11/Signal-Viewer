...
class Graph:
    ...
    def update_graph(self, data, current_index, window_width, graph1_color):
        ...
        if data is not None:
            ...
            #     # self.plot_widget.setXRange((self.index + window_width) * 0.001, (self.index + 2 * window_width) * 0.001)
            #     self.plot_widget.setLimits(xMin = (self.index + window_width) * 0.001, yMin=min(data), yMax=max(data))
            #     self.index += 1
            if current_index < 500:
                # At begin x-axis range from 0 to window_width
                self.plot_widget.setXRange(0, window_width * 0.001)   
